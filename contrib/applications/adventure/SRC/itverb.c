...
ivopen()
   ...
   if (object==0)
         {
         rspeak(28);                       /* nothing here with lock */
         return;
         }
   vopen();
   }
...
ivfoo()
   ...
   int k,msg;
   k = vocab(word1,3000);
   msg = 42;                                      /* nothing happens */
   if (foobar != 1 - k)
         {
         if (foobar != 0)
               msg = 151;              /* can't you read? start over */
         rspeak(msg);
         return;
         }
   foobar = k;
...
inventory()
   ...
   int msg;
   int i;
   msg = 98;                                /* not carrying anything */
   for (i = 1; i <= MAXOBJ; ++i)
      {
      if (i==BEAR || !toting(i))
            continue;
      if (msg)
            rspeak(99);               /* currently holding following */
      msg = 0;
      pspeak(i,-1);
      }
   if (toting(BEAR))
         msg = 141;                        /* being followed by bear */
   if (msg)
         rspeak(msg);
   }
/*
