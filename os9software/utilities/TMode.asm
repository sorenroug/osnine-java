...
*
 ...
 ttl OS9 Utility to change scf path parameters
 pag
**********
* Tmode [.path#] [{opts}]
*   Os-9 utility to change Scf path option bytes
* Author: Robert Doggett

* Default path is #0 (Std Input).
* If no options are given, a list of current
* values of all settable options is printed.
* The Os-9 Users Manual contains a list of
* the options, and their defaults.

* Note: if Tmode is used from a command file,
*  it is necessary to specify ".1" or ".2",
*  otherwise you are trying to change the mode
*  of an Rbf path, which doesn't make sense.
...
C$PDLIM set '/
 ...
 org 0
SETPTH rmb 1 Set's path number
NOTSET rmb 1 1="not", 0=not "not"
LINPTR rmb 2 Command line ptr
LINPOS rmb 1 Output line position
SIGNIF rmb 1 Significant digits
OPTBUF rmb 32 Pd option bytes
...
BINARY equ 1
 ...
 ifeq CPUType-COLOR
 fdb 21 Number of table entries (baud, no XON, XOFF)
 else
 fdb 22 Number of table entries (no baud)
 endc
